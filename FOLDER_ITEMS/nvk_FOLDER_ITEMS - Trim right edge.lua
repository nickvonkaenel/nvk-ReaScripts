...
local function prev_track_item_in_arrangeview(track, pos)
    ...
    if item and item.rgnend > start then
        return item
    end
end
run(function()
    ...
    for i, item in ipairs(items) do
        if i > 1 then
            if not item.mute then
                if item.e > init_end then
                    init_end = item.e
                end
                if item.s < init_pos then
                    init_pos = item.s
                end
            end
        end
    ...
    for i, item in ipairs(items) do
        ...
        if new_fade_out < 0 then
            new_fade_out = FADE_LENGTH_MIN
        end
        if i > 1 and item.s >= cursor_pos then
            item.automute = true
        else
            if i > 1 and item.automute and item.s < cursor_pos then
                item.automute = false
            end
