...
local function CloseTrackFX(track)
    ...
    for i = 0, fx - 1 do
        if reaper.TrackFX_GetOpen(track, i) then
            reaper.TrackFX_SetOpen(track, i, 0)
        end
    ...
    for i = 0, rec_fx - 1 do
        ...
        if reaper.TrackFX_GetOpen(track, i_rec) then
            reaper.TrackFX_SetOpen(track, i_rec, 0)
        end
        if reaper.TrackFX_GetRecChainVisible(track) ~= -1 then
            reaper.TrackFX_Show(track, i_rec, 0)
        end
    end
end
