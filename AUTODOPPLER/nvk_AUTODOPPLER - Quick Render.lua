...
function Run()
    selectorVal = 1
    if reaper.HasExtState(scrName, 'selector') then selectorVal = tonumber(reaper.GetExtState(scrName, 'selector')) end
    if
        not reaper.HasExtState(scrName, 'x')
        and not reaper.HasExtState(scrName, 'y')
        and reaper.CountSelectedTracks(0) > 0
    then
