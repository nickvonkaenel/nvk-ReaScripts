...
run(function()
    ...
    for i = 0, track.fxcount - 1 do
        ...
        if fxname:find('_AM_Playdium_Random_Midi_Velocity_Generator') then
            midirand_idx = i
        end
    end
    ...
    if not rs5k_idx then
        rs5k_idx = reaper.TrackFX_AddByName(mediatrack, 'ReaSamplOmatic5000 (Cockos)', false, -1)
    end
