...
local function get_folder(directoryPath)
    ...
    if r.APIExists("JS_Dialog_BrowseForFolder") then
        ...
        retval, folder = r.GetUserInputs("Path to save items", 1, "Paste path here:,extrawidth=500",
            directoryPath)
    ...
    if retval then
        r.SetExtState(scr.name, "path", folder, true)
    end
end
...
run(function()
    ...
    for i, item in ipairs(items) do
        ...
        if take ~= initTake then       --check if render finished successfully
            ...
            if path then
                CopyFile(path .. sep .. name, directoryPath .. sep .. name)
            end
        end
    end
end)
